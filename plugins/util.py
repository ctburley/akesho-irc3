...
class Plugin:
	...
	def disable_commands(self, target=None, data=None, **kw):
		...
		print("Disabling Commands In " + target)
		self.bot.goTime(6, self.reenable_commands, target)
	...
	def reenable_commands(self, channel):
		...
		print("Reenabling Commands In " + channel)
	...
	def timer_callback(self, call_id, meth, args):
		del self.timers[call_id]
		if len(args) > 0:
			meth(args)
		else:
			meth()
	...
	@irc3.extend
	def goTime(self, sec, meth, args=[]):
		"""Set a timer"""
		call_id = str(meth.__name__)+str(random.getrandbits(32))
		self.timers[call_id] = Timer(sec, self.timer_callback, [call_id, meth, args])
